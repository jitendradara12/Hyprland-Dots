...
function Yatline.string.get:hovered_file_extension(show_icon)
	...
	if hovered then
		...
		if show_icon then
			local matched = th and th.icon and th.icon:match(hovered)
			local icon = (matched and matched.text) or ""
...
function Yatline.string.get:tab_path(trimmed, max_length, trim_length)
	...
	local spec = cwd and cwd.spec

	local t = {}
	if spec and spec.is_search then
		t[#t + 1] = string.format("search: %s", spec.domain or "")
	end
	if filter then
...
function Yatline.string.get:search_query(key)
	...
	if spec and spec.is_search then
		return string.format("%s %s", key, spec.domain or "")
	else
		return ""
	end
end
...
function Yatline.coloreds.get:count(filter, zero_check)
	...
	if filter then
		...
		local cwd_spec = cx.active.current.cwd and cx.active.current.cwd.spec
		if cx.active.current.files.filter or (cwd_spec and cwd_spec.is_search) then
